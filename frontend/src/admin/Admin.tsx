...
export default function Admin() {
  ...
  function isOrderFlying(o: any) {
    const s = String(o.status || '').toLowerCase()
    return ['bắt đầu lấy đồ ăn', 'chuẩn bị giao hàng', 'đang giao đồ ăn bằng drone'].includes(s)
  }
        ...
        try {
          ...
          es.addEventListener('status', (ev: any) => {
            try {
              ...
              // Refresh drones to update status
              refreshDrones()
            } catch {}
          })
  ...
  function droneStatusColor(status?: string) {
    ...
    if (s === 'ready') return '#22c55e' // xanh lá
    if (s === 'waiting') return '#eab308' // vàng
    if (s === 'in_use' || s === 'inuse') return '#ef4444' // đỏ
    if (s === 'maintenance') return '#6b7280' // xám
    if (s === 'charging') return '#3b82f6' // xanh dương
    return '#9ca3af'
  }
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                          <select
                            ...
                            onChange={async e => {
                              ...
                              if (isOrderFlying(o) || armed[o.id]) {
                                alert('Không thể thay đổi drone')
                          ...
                          <button
                            className="btn small"
                            style={{ background: (armed[o.id] || isOrderFlying(o) ? 'var(--accent)' : ''), color: (armed[o.id] || isOrderFlying(o) ? '#fff' : '') }}
                            disabled={!hasDrone}
                            onClick={async () => {
                              if (!(o as any).droneId) { alert('Vui lòng chọn drone cho đơn này trước'); return }
                              if (armed[o.id] || isOrderFlying(o)) {
                                alert('Không thể thay đổi drone')
                                return
                              }
                              ...
                              if (next) {
                                try { 
                                  const dId = (o as any).droneId
                                  if (dId) {
                                    setDrones(prev => prev.map(d => d.id === dId ? { ...d, status: 'in_use' } : d))
                                  }
                                  await axios.post(`${ORDER_BASE.replace(/\/$/, '')}/orders/${o.id}/drone/arm`);
                                  setArmed(a => ({ ...a, [o.id]: next }))
                                  // Refresh drones immediately to update status
                                  await refreshDrones()
                                } catch {}
                              }
                            }}
