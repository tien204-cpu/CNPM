            ...
            if (data.type === 'route' && Array.isArray(data.path)) {
              ...
              if (data.append && routeRef.current) {
                ...
                if (data.initialDronePos && data.path.length > 0) {
                  ...
                  const restaurantPos = [data.path[0].lat, data.path[0].lng]
                  const initialLine = [droneStart, restaurantPos]
                  L.polyline(initialLine, { color: '#f59e0b', weight: 3, dashArray: '5, 10' }).addTo(mapRef.current)
                }
