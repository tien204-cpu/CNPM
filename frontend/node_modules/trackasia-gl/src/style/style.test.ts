...
import {type LayerSpecification, type GeoJSONSourceSpecification, type FilterSpecification, type SourceSpecification, type StyleSpecification, type SymbolLayerSpecification, type SkySpecification} from '@trackasia/trackasia-gl-style-spec';
...
describe('Style#loadJSON', () => {
    ...
    test('creates sources', async () => {
        ...
        await style.once('style.load');
        expect(style.sourceCaches['trackasia'] instanceof SourceCache).toBeTruthy();
    });
    ...
    test('sets up layer event forwarding', async () => {
        ...
        await style.once('style.load');
        style._layers.background.fire(new Event('error', {trackasia: true}));

        const e = await errorPromise;
        expect(e.layer).toEqual({id: 'background'});
        expect(e.trackasia).toBeTruthy();
    });
...
describe('Style#removeSource', () => {
    ...
    async function createStyleAndLoad(): Promise<Style> {
        ...
        style.loadJSON(createStyleJSON({
            'sources': {
                'trackasia-source': createGeoJSONSource()
            },
            'layers': [{
                'id': 'trackasia-layer',
                'type': 'circle',
                'source': 'trackasia-source',
    ...
    test('throws if source is in use', async () => {
        ...
        const promise =  style.once('error');
        style.removeSource('trackasia-source');
        const event = await promise;
        expect(event.error.message.includes('"trackasia-source"')).toBeTruthy();
        expect(event.error.message.includes('"trackasia-layer"')).toBeTruthy();
    });

    test('does not throw if source is not in use', async () => {
        const style = await createStyleAndLoad();
        const promise = style.once('error');
        style.removeLayer('trackasia-layer');
        style.removeSource('trackasia-source');
...
describe('Style#addLayer', () => {
    ...
    test('reloads source', async () => {
        ...
        const spy = vi.fn();
        style.sourceCaches['trackasia'].reload = spy;
        style.addLayer(layer);
        style.update({} as EvaluationParameters);
        expect(spy).toHaveBeenCalled();
    });
    ...
    test('#3895 reloads source (instead of clearing) if adding this layer with the same type, immediately after removing it', async () => {
        ...
        const spy = vi.fn();
        style.sourceCaches['trackasia'].reload = spy;
        style.sourceCaches['trackasia'].clearTiles =  () => { throw new Error('test failed'); };
    ...
    test('clears source (instead of reloading) if adding this layer with a different type, immediately after removing it', async () => {
        ...
        await waitForEvent(style, 'data', (e) => e.dataType === 'source' && e.sourceDataType === 'content');
        const spy = vi.fn();
        style.sourceCaches['trackasia'].reload =  () => { throw new Error('test failed'); };
        style.sourceCaches['trackasia'].clearTiles = spy;
        style.removeLayer('my-layer');
        style.addLayer(layer);
        style.update({} as EvaluationParameters);
        expect(spy).toHaveBeenCalled();
...
describe('Style#removeLayer', () => {
    ...
    test('tears down layer event forwarding', async () => {
        ...
        layer.fire(new Event('error', {trackasia: true}));
    });
...
describe('Style#queryRenderedFeatures', () => {
    ...
    beforeEach(() => new Promise<void>(callback => {
        ...
        function querytrackasiaFeatures(layers, serializedLayers, getFeatureState, queryGeom, cameraQueryGeom, scale, params) {
            const features = {
                'land': [{
                    type: 'Feature',
        ...
        style.on('style.load', () => {
            style.sourceCaches.trackasia.tilesIn = () => {
                return [{
                    tile: {queryRenderedFeatures: querytrackasiaFeatures} as unknown as Tile,
            ...
            style.sourceCaches.trackasia.transform = transform;
    ...
    test('does not query sources not implicated by `layers` parameter', () => {
        style.sourceCaches.trackasia.map.queryRenderedFeatures = vi.fn();
        style.queryRenderedFeatures([{x: 0, y: 0} as Point], {layers: ['land--other']}, transform);
        expect(style.sourceCaches.trackasia.map.queryRenderedFeatures).not.toHaveBeenCalled();
    });
