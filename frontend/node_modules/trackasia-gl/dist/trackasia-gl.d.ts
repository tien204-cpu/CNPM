...
import { Color, ColorArray, CompositeExpression, DiffCommand, DiffOperations, Feature, FeatureFilter, FeatureState, FilterSpecification, Formatted, FormattedSection, GeoJSONSourceSpecification, GlobalProperties, ICanonicalTileID, IMercatorCoordinate, ImageSourceSpecification, InterpolationType, LayerSpecification, LightSpecification, NumberArray, Padding, ProjectionSpecification, PromoteIdSpecification, PropertyValueSpecification, RasterDEMSourceSpecification, RasterSourceSpecification, ResolvedImage, SkySpecification, SourceExpression, SourceSpecification, SpriteSpecification, StylePropertyExpression, StylePropertySpecification, StyleSpecification, TerrainSpecification, TransitionSpecification, VariableAnchorOffsetCollection, VectorSourceSpecification, VideoSourceSpecification } from '@trackasia/trackasia-gl-style-spec';
 ...
 *
 * TrackAsia GL JS uses longitude, latitude coordinate order (as opposed to latitude, longitude) to match the
 * [GeoJSON specification](https://tools.ietf.org/html/rfc7946).
 *
 * Note that any TrackAsia GL JS method that accepts a `LngLat` object as an argument or option
 ...
 * ll.lng; // = -123.9749
 * ```
 * @see [Get coordinates of the mouse pointer](https://docs.track-asia.com/examples/mouse-position/)
 * @see [Display a popup](https://docs.track-asia.com/examples/popup/)
 * @see [Create a timeline animation](https://docs.track-asia.com/examples/timeline-animation/)
 */
 ...
 * ```
 * @see [Add a custom style layer](https://docs.track-asia.com/examples/custom-style-layer/)
 */
 ...
 *
 * @see [Add an animated icon to the map.](https://docs.track-asia.com/examples/add-image-animated/)
 *
 * @example
 * ```ts
 * let flashingSquare = {
 ...
 * });
 * ```
 * @see [Fit to the bounds of a LineString](https://docs.track-asia.com/examples/zoomto-linestring/)
 * @see [Fit a map to a bounding box](https://docs.track-asia.com/examples/fitbounds/)
 */
 ...
 * (See the [Style Specification](https://docs.track-asia.com/style-spec/sources/#image) for detailed documentation of options.)
 *
 * @group Sources
 *
 * @example
 ...
 *    type: 'image',
 *    url: 'https://track-asia.com/images/foo.png',
 *    coordinates: [
 *        [-76.54, 39.18],
 *        [-76.52, 39.18],
 *        [-76.52, 39.17],
 ...
 *    url: 'https://track-asia.com/images/bar.png',
 *    coordinates: [
 *        [-76.54335737228394, 39.18579907229748],
 *        [-76.52803659439087, 39.1838364847587],
 *        [-76.5295386314392, 39.17683392507606],
...
declare class SourceCache extends Evented {
	 ...
	 */
	getFeatureState(sourceLayer: string, featureId: number | string): import("@trackasia/trackasia-gl-style-spec").FeatureState;
	/**
...
/**
 * This type contains all data necessary to project a tile to screen in TrackAsia's shader system.
...
export type QueryRenderedFeaturesOptions = {
	/**
	 * An array or set of [style layer IDs](https://docs.track-asia.com/style-spec/layers/#id) for the query to inspect.
	 * Only features within these layers will be returned. If this parameter is undefined, all layers will be checked.
	 */
	layers?: Array<string> | Set<string>;
	/**
	 * A [filter](https://docs.track-asia.com/style-spec/layers/#filter) to limit query results.
	 */
	filter?: FilterSpecification;
	/**
...
export type QuerySourceFeatureOptions = {
	...
	sourceLayer?: string;
	/**
	 * A [filter](https://docs.track-asia.com/style-spec/layers/#filter)
	 * to limit query results.
	 */
	filter?: FilterSpecification;
	/**
	 * Whether to check if the [parameters.filter] conforms to the TrackAsia Style Specification. Disabling validation is a performance optimization that should only be used if you have previously validated the values you will be passing to this function.
	 * @defaultValue true
	 */
	validate?: boolean;
};
...
declare class LineStyleLayer extends StyleLayer {
	...
	constructor(layer: LayerSpecification);
	_handleSpecialPaintPropertyUpdate(name: string): void;
	gradientExpression(): import("@trackasia/trackasia-gl-style-spec").StylePropertyExpression;
	recalculate(parameters: EvaluationParameters, availableImages: Array<string>): void;
...
export type CustomRenderMethodInput = {
	...
	*/
	...
	shaderData: {
		...
		variantName: string;
		/**
		 * The prelude code to add to the vertex shader to access TrackAsia's `projectTile` projection function.
		 * Depends on current projection.
		 * @example
		 * ```
	...
	/**
	 * Uniforms that should be passed to the vertex shader, if TrackAsia's projection code is used.
...
/**
 * An interface the implementations of which are used internally by TrackAsia to handle different projections.
 */
export interface Projection {
	 ...
	 * Optimized rendering paths for non-subdivided geometry might be used throughout TrackAsia.
	 ...
	 * Then globe projection might start reporting the mercator shader variant name to make TrackAsia use faster mercator shaders.
	 */
	get shaderVariantName(): string;
	/**
	 * A `#define` macro that is injected into every TrackAsia shader that uses this projection.
	 * @example
	...
	/**
	 * Vertex shader code that is injected into every TrackAsia vertex shader that uses this projection.
	 */
...
export type StyleSetterOptions = {
	/**
	 * Whether to check if the filter conforms to the TrackAsia Style Specification. Disabling validation is a performance optimization that should only be used if you have previously validated the values you will be passing to this function.
	 */
	validate?: boolean;
};
/**
 ...
 * @example
 * ```ts
 * map.setStyle('https://static.track-asia.com/demotiles/style.json', {
 *   transformStyle: (previousStyle, nextStyle) => ({
 *       ...nextStyle,
 *       // make relative sprite path like "../sprite" absolute
 *       sprite: new URL(nextStyle.sprite, "https://static.track-asia.com/demotiles/styles/osm-bright-gl-style/sprites/").href,
 *       // make relative glyphs path like "../fonts/{fontstack}/{range}.pbf" absolute
 *       glyphs: new URL(nextStyle.glyphs, "https://static.track-asia.com/demotiles/font/").href,
 *       sources: {
 *           // make relative vector url like "../../" absolute
 *           ...nextStyle.sources.map(source => {
 *              if (source.url) {
	 *              source.url = new URL(source.url, "https://static.track-asia.com/demotiles/tiles/osm-bright-gl-style/");
 *              }
 *              return source;
 *           }),
...
export declare class Style extends Evented {
	...
	getFeatureState(target: FeatureIdentifier): import("@trackasia/trackasia-gl-style-spec").FeatureState;
	getTransition(): {
		duration: number;
		delay: number;
	} & import("@trackasia/trackasia-gl-style-spec").TransitionSpecification;
 ...
 * });
 * ```
 * @see [Set pitch and bearing](https://docs.track-asia.com/examples/set-perspective/)
 * @see [Jump to a series of locations](https://docs.track-asia.com/examples/jump-to/)
 * @see [Fly to a location](https://docs.track-asia.com/examples/flyto/)
 * @see [Display buildings in 3D](https://docs.track-asia.com/examples/3d-buildings/)
 */
...
declare abstract class Camera extends Evented {
	 ...
	 * @param options - Options object
	 * @param eventData - Additional properties to be added to event objects of events triggered by this method.
	 * @see [Navigate the map with game-like controls](https://docs.track-asia.com/examples/game-controls/)
	 */
	 ...
	 * ```
	 * @see [Update a feature in realtime](https://docs.track-asia.com/examples/live-update-feature/)
	 */
	 ...
	 * ```
	 * @see [Fly to a location](https://docs.track-asia.com/examples/flyto/)
	 * @see [Slowly fly to a location](https://docs.track-asia.com/examples/flyto-options/)
	 * @see [Fly to a location based on scroll position](https://docs.track-asia.com/examples/scroll-fly-to/)
	 */
	 ...
	 * This method should be used for proper positioning of custom 3d objects, as explained [here](https://docs.track-asia.com/examples/add-3d-model-with-terrain/)
...
export type MapLayerEventType = {
	 ...
	 *
	 * @see [Measure distances](https://docs.track-asia.com/examples/measure/)
	 * @see [Center the map on a clicked symbol](https://docs.track-asia.com/examples/center-on-symbol/)
	 */
	 ...
	 * @see [Create a draggable point](https://docs.track-asia.com/examples/drag-a-point/)
	 */
	 ...
	 *
	 * @see [Get coordinates of the mouse pointer](https://docs.track-asia.com/examples/mouse-position/)
	 * @see [Highlight features under the mouse pointer](https://docs.track-asia.com/examples/hover-styles/)
	 * @see [Display a popup on over](https://docs.track-asia.com/examples/popup-on-hover/)
	 * @see [Animate symbol to follow the mouse](https://docs.track-asia.com/examples/animate-symbol-to-follow-mouse/)
	 */
	 ...
	 *
	 * @see [Center the map on a clicked symbol](https://docs.track-asia.com/examples/center-on-symbol/)
	 * @see [Display a popup on click](https://docs.track-asia.com/examples/popup-on-click/)
	 */
	 ...
	 * @see [Get coordinates of the mouse pointer](https://docs.track-asia.com/examples/mouse-position/)
	 * @see [Highlight features under the mouse pointer](https://docs.track-asia.com/examples/hover-styles/)
	 * @see [Display a popup on hover](https://docs.track-asia.com/examples/popup-on-hover/)
	 */
...
export type MapEventType = {
	 ...
	 *
	 * @see [Draw GeoJSON points](https://docs.track-asia.com/examples/geojson-markers/)
	 * @see [Add live realtime data](https://docs.track-asia.com/examples/live-geojson/)
	 * @see [Animate a point](https://docs.track-asia.com/examples/animate-point-along-line/)
	 */
	load: TrackAsiaEvent;
	/**
	 ...
	 */
	render: TrackAsiaEvent;
	/**
	 * Fired immediately after the map has been resized.
	 */
	resize: TrackAsiaEvent;
	/**
	 ...
	 * @see [Display HTML clusters with custom properties](https://docs.track-asia.com/examples/cluster-html/)
	 */
	data: MapDataEvent;
	 ...
	 * @see [Generate and add a missing icon to the map](https://docs.track-asia.com/examples/add-image-missing-generated/)
	 */
	 ...
	 */
	boxzoomcancel: TrackAsiaZoomEvent;
	/**
	 * Fired when a "box zoom" interaction starts. See {@link BoxZoomHandler}.
	 */
	boxzoomstart: TrackAsiaZoomEvent;
	/**
	 * Fired when a "box zoom" interaction ends.  See {@link BoxZoomHandler}.
	 */
	boxzoomend: TrackAsiaZoomEvent;
	/**
	 ...
	 *
	 */
	movestart: TrackAsiaEvent<MouseEvent | TouchEvent | WheelEvent | undefined>;
	/**
	 ...
	 * @see [Display HTML clusters with custom properties](https://docs.track-asia.com/examples/cluster-html/)
	 */
	move: TrackAsiaEvent<MouseEvent | TouchEvent | WheelEvent | undefined>;
	/**
	 ...
	 * @see [Display HTML clusters with custom properties](https://docs.track-asia.com/examples/cluster-html/)
	 */
	moveend: TrackAsiaEvent<MouseEvent | TouchEvent | WheelEvent | undefined>;
	/**
	 * Fired just before the map begins a transition from one zoom level to another,
	 * as the result of either user interaction or methods such as {@link Map#flyTo}.
	 */
	zoomstart: TrackAsiaEvent<MouseEvent | TouchEvent | WheelEvent | undefined>;
	/**
	 * Fired repeatedly during an animated transition from one zoom level to another,
	 * as the result of either user interaction or methods such as {@link Map#flyTo}.
	 */
	zoom: TrackAsiaEvent<MouseEvent | TouchEvent | WheelEvent | undefined>;
	/**
	 * Fired just after the map completes a transition from one zoom level to another,
	 * as the result of either user interaction or methods such as {@link Map#flyTo}.
	 */
	zoomend: TrackAsiaEvent<MouseEvent | TouchEvent | WheelEvent | undefined>;
	/**
	 * Fired when a "drag to rotate" interaction starts. See {@link DragRotateHandler}.
	 */
	rotatestart: TrackAsiaEvent<MouseEvent | TouchEvent | undefined>;
	/**
	 * Fired repeatedly during a "drag to rotate" interaction. See {@link DragRotateHandler}.
	 */
	rotate: TrackAsiaEvent<MouseEvent | TouchEvent | undefined>;
	/**
	 * Fired when a "drag to rotate" interaction ends. See {@link DragRotateHandler}.
	 */
	rotateend: TrackAsiaEvent<MouseEvent | TouchEvent | undefined>;
	/**
	 * Fired when a "drag to pan" interaction starts. See {@link DragPanHandler}.
	 */
	dragstart: TrackAsiaEvent<MouseEvent | TouchEvent | undefined>;
	/**
	 * Fired repeatedly during a "drag to pan" interaction. See {@link DragPanHandler}.
	 */
	drag: TrackAsiaEvent<MouseEvent | TouchEvent | undefined>;
	/**
	 * Fired when a "drag to pan" interaction ends. See {@link DragPanHandler}.
	 * @see [Create a draggable marker](https://docs.track-asia.com/examples/drag-a-marker/)
	 */
	dragend: TrackAsiaEvent<MouseEvent | TouchEvent | undefined>;
	/**
	 * Fired whenever the map's pitch (tilt) begins a change as
	 * the result of either user interaction or methods such as {@link Map#flyTo} .
	 */
	pitchstart: TrackAsiaEvent<MouseEvent | TouchEvent | undefined>;
	/**
	 * Fired repeatedly during the map's pitch (tilt) animation between
	 * one state and another as the result of either user interaction
	 * or methods such as {@link Map#flyTo}.
	 */
	pitch: TrackAsiaEvent<MouseEvent | TouchEvent | undefined>;
	/**
	 * Fired immediately after the map's pitch (tilt) finishes changing as
	 * the result of either user interaction or methods such as {@link Map#flyTo}.
	 */
	pitchend: TrackAsiaEvent<MouseEvent | TouchEvent | undefined>;
	/**
	 ...
	 */
	cooperativegestureprevented: TrackAsiaEvent<WheelEvent | TouchEvent> & {
...
/**
 * The base event for TrackAsia
 *
 * @group Event Related
 */
export type TrackAsiaEvent<TOrig = unknown> = {
 ...
 * @group Event Related
 */
export type MapStyleDataEvent = TrackAsiaEvent & {
	dataType: "style";
};
/**
 ...
 * @group Event Related
 */
export type MapSourceDataEvent = TrackAsiaEvent & {
	dataType: "source";
	/**
	...
	/**
	 * The [style spec representation of the source](https://docs.track-asia.com/style-spec/sources/) if the event has a `dataType` of `source`.
	 */
	source: SourceSpecification;
	sourceId: string;
 ...
 * ```
 */
export declare class MapMouseEvent extends Event$1 implements TrackAsiaEvent<MouseEvent> {
	/**
	 * The event type
	 */
 ...
 * @group Event Related
 */
export declare class MapTouchEvent extends Event$1 implements TrackAsiaEvent<TouchEvent> {
	/**
	 * The event type.
	 */
...
/**
 * A `TrackAsiaZoomEvent` is the event type for the boxzoom-related map events emitted by the {@link BoxZoomHandler}.
 *
 * @group Event Related
 */
export type TrackAsiaZoomEvent = {
	/**
 ...
 * - `'style'`: The [style](https://docs.track-asia.com/style-spec/) used by the map
 *
 ...
 * @see [Generate and add a missing icon to the map](https://docs.track-asia.com/examples/add-image-missing-generated/)
 */
export type MapStyleImageMissingEvent = TrackAsiaEvent & {
 ...
 * });
 * ```
 * @see [Example: cooperative gestures](https://docs.track-asia.com/examples/cooperative-gestures/)
 **/
...
export type MapOptions = {
	...
	interactive?: boolean;
	/**
	 * The HTML element in which TrackAsia GL JS will render the map, or the element's string `id`. The specified element must have no children.
	 */
	 ...
	 * Note: showing the logo of TrackAsia is not required for using TrackAsia.
	 * @defaultValue compact: true, customAttribution: "TrackAsia ...".
	 */
	attributionControl?: false | AttributionControlOptions;
	/**
	 * If `true`, the TrackAsia logo will be shown.
	 */
	trackasiaLogo?: boolean;
	/**
	 * A string representing the position of the TrackAsia wordmark on the map. Valid options are `top-left`,`top-right`, `bottom-left`, or `bottom-right`.
	 ...
	 * `contextType` can be set to `webgl2` or `webgl` to force a WebGL version. Not setting it, Trackasia will do it's best to get a suitable context.
	...
	trackResize?: boolean;
	/**
	 * The initial geographical centerpoint of the map. If `center` is not specified in the constructor options, TrackAsia GL JS will look for it in the map's style object. If it is not specified in the style, either, it will default to `[0, 0]` Note: TrackAsia GL JS uses longitude, latitude coordinate order (as opposed to latitude, longitude) to match GeoJSON.
	 * @defaultValue [0, 0]
	 */
	center?: LngLatLike;
	/**
	...
	elevation?: number;
	/**
	 * The initial zoom level of the map. If `zoom` is not specified in the constructor options, TrackAsia GL JS will look for it in the map's style object. If it is not specified in the style, either, it will default to `0`.
	 * @defaultValue 0
	 */
	zoom?: number;
	/**
	 * The initial bearing (rotation) of the map, measured in degrees counter-clockwise from north. If `bearing` is not specified in the constructor options, TrackAsia GL JS will look for it in the map's style object. If it is not specified in the style, either, it will default to `0`.
	 * @defaultValue 0
	 */
	bearing?: number;
	/**
	 * The initial pitch (tilt) of the map, measured in degrees away from the plane of the screen (0-85). If `pitch` is not specified in the constructor options, TrackAsia GL JS will look for it in the map's style object. If it is not specified in the style, either, it will default to `0`. Values greater than 60 degrees are experimental and may result in rendering issues. If you encounter any, please raise an issue with details in the TrackAsia project.
	 * @defaultValue 0
	 */
	pitch?: number;
	/**
	 * The initial roll angle of the map, measured in degrees counter-clockwise about the camera boresight. If `roll` is not specified in the constructor options, TrackAsia GL JS will look for it in the map's style object. If it is not specified in the style, either, it will default to `0`.
	 * @defaultValue 0
	 */
	roll?: number;
	/**
	 ...
	 * For these characters, font settings from the map's style will be ignored, except for font-weight keywords (light/regular/medium/bold).
	 * Set to `false`, to enable font settings from the map's style for these glyph ranges.
	 * The purpose of this option is to avoid bandwidth-intensive glyph server requests. (See [Use locally generated ideographs](https://docs.track-asia.com/examples/local-ideographs).)
	 * @defaultValue 'sans-serif'
	 */
	localIdeographFontFamily?: string | false;
	/**
	 * The map's TrackAsia style. This must be a JSON object conforming to
	 * the schema described in the [TrackAsia Style Specification](https://docs.track-asia.com/style-spec/),
 ...
 * Then TrackAsia GL JS initializes the map on the page and returns your `Map` object.
 *
 * @group Main
 *
 * @example
 ...
 * });
 * ```
 * @see [Display a map](https://docs.track-asia.com/examples/simple-map/)
 */
declare class Map$1 extends Camera {
	 ...
	 * ```
	 * @see [Display map navigation controls](https://docs.track-asia.com/examples/navigation/)
	 */
	 ...
	 *
	 * @param minPitch - The minimum pitch to set (0-180). Values greater than 60 degrees are experimental and may result in rendering issues. If you encounter any, please raise an issue with details in the TrackAsia project.
	 ...
	 *
	 * @param maxPitch - The maximum pitch to set (0-180). Values greater than 60 degrees are experimental and may result in rendering issues. If you encounter any, please raise an issue with details in the TrackAsia project.
	 ...
	 * ```
	 * @see [Render world copies](https://docs.track-asia.com/examples/render-world-copies/)
	 */
	 ...
	 *     source: {
	 *       type: 'vector',
	 *       url: 'https://docs.track-asia.com/style-spec/'
	 *     },
	 *     'source-layer': 'poi_label',
	 *     type: 'circle',
	 *     paint: {
	 *       // TrackAsia Style Specification paint properties
	 *     },
	 *     layout: {
	 *       // TrackAsia Style Specification layout properties
	 *     }
	 *   });
	 * });
	 * ```
	 ...
	 * });
	 * ```
	 * @see [Display popup on click](https://docs.track-asia.com/examples/popup-on-click/)
	 * @see [Center the map on a clicked symbol](https://docs.track-asia.com/examples/center-on-symbol/)
	 * @see [Create a hover effect](https://docs.track-asia.com/examples/hover-styles/)
	 * @see [Create a draggable marker](https://docs.track-asia.com/examples/drag-a-point/)
	 */
	 ...
	 * ```
	 * @see [Get features under the mouse pointer](https://docs.track-asia.com/examples/queryrenderedfeatures/)
	 */
	 ...
	 * [TrackAsia Style Specification](https://docs.track-asia.com/style-spec/), or a URL to such JSON.
	 * @param options - The options object.
	 *
	 * @example
	 * ```ts
	...
	/**
	 * Returns the map's TrackAsia style object, a JSON object which can be used to recreate the map's style.
	 *
	 ...
	 * TrackAsia Style Specification's [source definition](https://docs.track-asia.com/style-spec/sources) or
	 * {@link CanvasSourceSpecification}.
	 * @example
	 * ```ts
	 * map.addSource('my-data', {
	 *   type: 'vector',
	 *   url: 'https://static.track-asia.com/demotiles/tiles/tiles.json'
	 * });
	 * ```
	 * @example
	 * ```ts
	 ...
	 * });
	 * ```
	 * @see GeoJSON source: [Add live realtime data](https://docs.track-asia.com/examples/live-geojson/)
	 */
	 ...
	 * A list of options for each source type is available on the TrackAsia Style Specification's
	 * [Sources](https://docs.track-asia.com/style-spec/sources/) page.
	 * @example
	 * ```ts
	 ...
	 * ```
	 * @see [Modify Level of Detail behavior](https://docs.track-asia.com/examples/lod-control/)
	 ...
	 * [`icon-image`](https://docs.track-asia.com/style-spec/layers/#layout-symbol-icon-image),
	 * [`background-pattern`](https://docs.track-asia.com/style-spec/layers/#paint-background-background-pattern),
	 * [`fill-pattern`](https://docs.track-asia.com/style-spec/layers/#paint-fill-fill-pattern),
	 * or [`line-pattern`](https://docs.track-asia.com/style-spec/layers/#paint-line-line-pattern).
	 *
	 ...
	 * });
	 * ```
	 * @see Use `HTMLImageElement`: [Add an icon to the map](https://docs.track-asia.com/examples/add-image/)
	 * @see Use `ImageData`: [Add a generated icon to the map](https://docs.track-asia.com/examples/add-image-generated/)
	 */
	 ...
	 * ```
	 * @see [Add an icon to the map](https://docs.track-asia.com/examples/add-image/)
	 */
	...
	listImages(): Array<string>;
	/**
	 * Adds a [TrackAsia style layer](https://docs.track-asia.com/style-spec/layers)
	 * to the map's style.
	 *
	 * A layer defines how data from a specified source will be styled. Read more about layer types
	 * and available paint and layout properties in the [TrackAsia Style Specification](https://docs.track-asia.com/style-spec/layers).
	 *
	 * @param layer - The layer to add,
	 * conforming to either the TrackAsia Style Specification's [layer definition](https://docs.track-asia.com/style-spec/layers) or,
	 ...
	 * The TrackAsia Style Specification's layer definition is appropriate for most layers.
	 *
	 ...
	 * }, 'cities');
	 * ```
	 * @see [Create and style clusters](https://docs.track-asia.com/examples/cluster/)
	 * @see [Add a vector tile source](https://docs.track-asia.com/examples/vector-source/)
	 * @see [Add a WMS source](https://docs.track-asia.com/examples/wms/)
	 */
	 ...
	 * ```
	 * @see [Filter symbols by toggling a list](https://docs.track-asia.com/examples/filter-markers/)
	 * @see [Filter symbols by text input](https://docs.track-asia.com/examples/filter-markers-by-input/)
	 */
	getLayer(id: string): StyleLayer | undefined;
	/**
	 * Return the ids of all layers currently in the style, including custom layers, in order.
	 ...
	 * [minimum zoom level](https://docs.track-asia.com/style-spec/layers/#minzoom)
	 * and [maximum zoom level](https://docs.track-asia.com/style-spec/layers/#maxzoom))
	 ...
	 * @param filter - The filter, conforming to the TrackAsia Style Specification's
	 * [filter definition](https://docs.track-asia.com/style-spec/layers/#filter).  If `null` or `undefined` is provided, the function removes any existing filter from the layer.
	 * @param options - Options object.
	 *
	 * @example
	 ...
	 * Must be of a type appropriate for the property, as defined in the [TrackAsia Style Specification](https://docs.track-asia.com/style-spec/).
	 ...
	 * ```
	 * @see [Change a layer's color with buttons](https://docs.track-asia.com/examples/color-switcher/)
	 * @see [Create a draggable point](https://docs.track-asia.com/examples/drag-a-point/)
	 */
	 ...
	 * @param value - The value of the layout property. Must be of a type appropriate for the property, as defined in the [TrackAsia Style Specification](https://docs.track-asia.com/style-spec/).
	 * @param options - The options object.
	 * @example
	 * ```ts
	 ...
	 *
	 * @param glyphsUrl - Glyph URL to set. Must conform to the [TrackAsia Style Specification](https://docs.track-asia.com/style-spec/glyphs/).
	 * @param options - Options object.
	 * @example
	 * ```ts
	 * map.setGlyphs('https://static.track-asia.com/demotiles/font/{fontstack}/{range}.pbf');
	 * ```
	 */
	 ...
	 *
	 * @param light - Light properties to set. Must conform to the [TrackAsia Style Specification](https://docs.track-asia.com/style-spec/light).
	 * @param options - Options object.
	 *
	 * @example
	 * ```ts
	 ...
	 *
	 * @param sky - Sky properties to set. Must conform to the [TrackAsia Style Specification](https://docs.track-asia.com/style-spec/sky/).
	 * @param options - Options object.
	 *
	 * @example
	 * ```ts
	 ...
	 * - For vector or GeoJSON sources, using the [`promoteId`](https://docs.track-asia.com/style-spec/sources/#promoteid) option at the time the source is defined.
	 * - For GeoJSON sources, using the [`generateId`](https://docs.track-asia.com/style-spec/sources/#generateid) option to auto-assign an `id` based on the feature's index in the source data. If you change feature data using `map.getSource('some id').setData(..)`, you may need to re-apply state taking into account updated `id` values.
	 *
	 * _Note: You can use the [`feature-state` expression](https://docs.track-asia.com/style-spec/expressions/#feature-state) to access the values in a feature's state object for the purposes of styling._
	 *
	 ...
	 * Features are identified by their `feature.id` attribute, which can be any number or string.
	 *
	 * _Note: To access the values in a feature's state object for the purposes of styling the feature, use the [`feature-state` expression](https://docs.track-asia.com/style-spec/expressions/#feature-state)._
	 *
	 * @param feature - Feature identifier. Feature objects returned from
	 * {@link Map#queryRenderedFeatures} or event handlers can be used as feature identifiers.
	 ...
	 * map.triggerRepaint();
	 * ```
	 * @see [Add a 3D model](https://docs.track-asia.com/examples/add-3d-model/)
	 * @see [Add an animated icon to the map](https://docs.track-asia.com/examples/add-image-animated/)
	 */
	triggerRepaint(): void;
 ...
 * @see [Display a popup on hover](https://docs.track-asia.com/examples/popup-on-hover/)
 * @see [Display a popup on click](https://docs.track-asia.com/examples/popup-on-click/)
 * @see [Attach a popup to a marker instance](https://docs.track-asia.com/examples/set-popup/)
 *
 * ## Events
 *
...
export declare class Popup extends Evented {
	 ...
	 *
	 * @param map - The TrackAsia GL JS map to add the popup to.
	 * @example
	 * ```ts
	 * new Popup()
	 *   .setLngLat([0, 0])
	 ...
	 * @see [Display a popup on hover](https://docs.track-asia.com/examples/popup-on-hover/)
	 * @see [Display a popup on click](https://docs.track-asia.com/examples/popup-on-click/)
	 * @see [Show polygon information on click](https://docs.track-asia.com/examples/polygon-popup-on-click/)
	 */
	addTo(map: Map$1): this;
	/**
 ...
 *   .addTo(map);
 * ```
 * @see [Add custom icons with Markers](https://docs.track-asia.com/examples/custom-marker-icons/)
 * @see [Create a draggable Marker](https://docs.track-asia.com/examples/drag-a-marker/)
 *
 * ## Events
 *
...
export declare class Marker extends Evented {
	 ...
	 * @param map - The TrackAsia GL JS map to add the marker to.
	 * @example
	 * ```ts
	 * let marker = new Marker()
	 *   .setLngLat([30.5, 50.5])
 ...
 *
 * ## State Diagram
 * ![GeolocateControl state diagram](https://github.com/track-asia/trackasia-gl-js/assets/3269297/78e720e5-d781-4da8-9803-a7a0e6aaaa9f)
 *
 * @group Markers and Controls
 *
 * @example
 ...
 * }));
 * ```
 * @see [Locate the user](https://docs.track-asia.com/examples/locate-user/)
 *
 * ## Events
 *
 ...
 * ```
 * @see [View a fullscreen map](https://docs.track-asia.com/examples/fullscreen/)
 *
 * ## Events
 *
 ...
 * ```
 *
 * @see [Display a globe with a fill extrusion layer](https://docs.track-asia.com/examples/globe-fill-extrusion/)
 */
 ...
 * (See the [Style Specification](https://docs.track-asia.com/style-spec/sources/#geojson) for detailed documentation of options.)
 *
 * @group Sources
 *
 * @example
 ...
 * ```
 * @see [Draw GeoJSON points](https://docs.track-asia.com/examples/geojson-markers/)
 * @see [Add a GeoJSON line](https://docs.track-asia.com/examples/geojson-line/)
 * @see [Create a heatmap from points](https://docs.track-asia.com/examples/heatmap-layer/)
 * @see [Create and style clusters](https://docs.track-asia.com/examples/cluster/)
 */
...
/**
 * A source containing raster tiles (See the [Style Specification](https://docs.track-asia.com/style-spec/) for detailed documentation of options.)
 *
 * @group Sources
 *
 * @example
 ...
 * });
 * ```
 * @see [Add a raster tile source](https://docs.track-asia.com/examples/map-tiles/)
 * @see [Add a WMS source](https://docs.track-asia.com/examples/wms/)
 * @see [Display a satellite map](https://docs.track-asia.com/examples/satellite-map/)
 */
export declare class RasterTileSource extends Evented implements Source {
	 ...
	 *
	 * @param tiles - An array of one or more tile source URLs, as in the raster tiles spec (See the [Style Specification](https://docs.track-asia.com/style-spec/)
	 */
...
/**
 * A source containing raster DEM tiles (See the [Style Specification](https://docs.track-asia.com/style-spec/) for detailed documentation of options.)
 ...
 * });
 * ```
 * @see [3D Terrain](https://docs.track-asia.com/examples/3d-terrain/)
 */
 ...
 * (See the [Style Specification](https://docs.track-asia.com/style-spec/) for detailed documentation of options.)
 *
 * @group Sources
 *
 * @example
 ...
 * @example
 * ```ts
 * map.getSource('some id').setUrl("https://static.track-asia.com/demotiles/tiles/tiles.json");
 * ```
 *
 * @example
 * ```ts
 ...
 * (See the [Style Specification](https://docs.track-asia.com/style-spec/sources/#video) for detailed documentation of options.)
 *
 * @group Sources
 *
 * @example
 ...
 * map.removeSource('some id');  // remove
 * ```
 * @see [Add a video](https://docs.track-asia.com/examples/video-on-a-map/)
 *
 ...
 * @param lazy - If set to `true`, trackasia will defer loading the plugin until rtl text is encountered,
 * rtl text will then be rendered only after the plugin finishes loading.
 * @example
 * ```ts
 * setRTLTextPlugin('https://unpkg.com/@mapbox/mapbox-gl-rtl-text@0.3.0/dist/mapbox-gl-rtl-text.js', false);
 * ```
 * @see [Add support for right-to-left scripts](https://docs.track-asia.com/examples/mapbox-gl-rtl-text/)
 */
...
export type * from "@trackasia/trackasia-gl-style-spec";
...
export as namespace trackasiagl;
