...
describe('diff', () => {
    ...
    test('global metadata', () => {
        expect(diff({} as StyleSpecification, {
            metadata: {'trackasia:author': 'nobody'}
        } as StyleSpecification)).toEqual([]);
    });

    test('layer metadata', () => {
        expect(diff({
            layers: [{id: 'a', metadata: {'trackasia:group': 'Group Name'}}]
        } as StyleSpecification, {
            layers: [{id: 'a', metadata: {'trackasia:group': 'Another Name'}}]
        } as StyleSpecification)).toEqual([]);
    });
    ...
    test('add terrain', () => {
        ...
        } as StyleSpecification,
        {
            ...
            {command: 'setTerrain', args: [{source: 'trackasia-dem', exaggeration: 1.5}]},
        ]);
    });
